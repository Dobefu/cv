    ...
    >
      <div className="mx-auto grid max-w-5xl grid-cols-1 items-center gap-5 text-center md:grid-cols-3">
        <div className="md:text-start">
          <LocaleLink
            aria-label={appName}
            className="flex-none text-xl font-medium text-zinc-800 dark:text-white"
            href="/"
          >
            {appName}
          </LocaleLink>
          ...
          {footerLinks.map((footerLink) => {
            ...
            return (
              <LinkTag
                ...
                className="gap-2 text-sm text-gray-600 transition-colors hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200"
                href={footerLink.to}
                key={footerLink.to}
              >
                {footerLink.title}
