    ...
    <footer className="w-full bg-zinc-100 px-4 py-10 shadow-[0_-4px_6px_-1px_rgb(0,0,0,.1)] dark:bg-zinc-950 print:hidden">
      <div className="mx-auto grid max-w-5xl grid-cols-1 items-center gap-5 text-center md:grid-cols-3">
        <div className="md:text-start">
          <Link
            aria-label={appName}
            className="flex-none text-xl font-semibold text-zinc-800 dark:text-white dark:focus:outline-hidden dark:focus:ring-1 dark:focus:ring-gray-600"
            href="/"
          >
            {appName}
          </Link>
          ...
          {footerLinks.map((footerLink) => {
            ...
            return (
              <LinkTag
                ...
                className="gap-x-2 text-sm text-gray-600 transition-colors hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 dark:focus:outline-hidden dark:focus:ring-1 dark:focus:ring-gray-600"
                href={footerLink.to}
                key={footerLink.to}
              >
                {footerLink.title}
